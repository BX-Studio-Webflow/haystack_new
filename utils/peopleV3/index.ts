...
export async function peoplePageCode({ dataSource }: { dataSource: 'live' | 'dev' }) {
  ...
  async function getPerson(slug: string) {
    try {
      ...
      personCards.forEach((personCard) => {
        const personName = personCard.querySelector<HTMLHeadingElement>(`[dev-target=person-name]`);
        const personEmail = personCard.querySelector<HTMLLinkElement>(`[dev-target=email-link]`);
        const personTitle =
          personCard.querySelector<HTMLParagraphElement>(`[dev-target=person-title]`);
        const personBio = personCard.querySelector<HTMLParagraphElement>(`[dev-target=person-bio]`);
        ...
        // );
        const personInput =
          personImageWrapper?.querySelector<HTMLInputElement>(`[dev-target=person-input]`);
        ...
        personCompanyLink!.href = `${route}/company/` + person.company_details.slug;
  ...
  function insightSearch(personSlug: string, cb1?: () => void, cb2?: () => void) {
    cb1 && cb1();
    getPersonInsights(personSlug, {
      orderBy: sortObject.orderBy,
      sortBy: sortObject.sortBy,
