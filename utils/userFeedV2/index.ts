...
export async function userFeedCode({ dataSource }: { dataSource: 'live' | 'dev' }) {
  ...
  const userFeedSearchInputs = qsa<HTMLInputElement>('[dev-target=user-feed-search]');
  ...
  const lsFollowingTabFeed = localStorage.getItem('userFeedDataAllFollowingTab');
  const lsFavouriteTabFeed = localStorage.getItem('userFeedDataAllFavouriteTab');
  ...
  if (lsFollowingTabFeed && userFollowingAndFavourite) {
    ...
    //   userFollowingAndFavourite
    // );
    paginationLogic(JSON.parse(lsFollowingTabFeed) as MergedResult, 'following');
  }
  if (lsFavouriteTabFeed && userFollowingAndFavourite) {
    ...
    //   userFollowingAndFavourite
    // );
    paginationLogic(JSON.parse(lsFavouriteTabFeed) as MergedResult, 'favourite');
  }
  ...
  function userFeedInit() {
    ...
    getUserFeedData('/all-tab-insight-company-people_0', { type: 'all' }, allTabsTarget);
    getUserFeedData(
  ...
  async function getUserFeedData(
    ...
    try {
      ...
      if (
        ...
        localStorage.setItem('userFeedDataAllTab', JSON.stringify(mergedResults));
      }
      if (
        ...
        localStorage.setItem('userFeedDataAllFollowingTab', JSON.stringify(mergedResults));
      }
      if (
        ...
        localStorage.setItem('userFeedDataAllFavouriteTab', JSON.stringify(mergedResults));
      }
      userFollowingAndFavourite &&
        initUserFeedData(mergedResults, target, userFollowingAndFavourite);

      endPoint === '/all-tab-insight-company-people_0' && paginationLogic(mergedResults, 'all');
      endPoint === '/following-tab-insight-company-people_0' &&
  ...
  function initUserFeedData(
    ...
    qsa(`[tab-section=${feedSectionType}]`).forEach((section) => {
      ...
      if (countDiv) {
        countDiv.textContent = `(${mergedData.returnTypeCount[filterType].toString()})`;
      }
    });
    mergedData.items.forEach((data) => {
      ...
      const userFeedType = newInsight.querySelector<HTMLDivElement>(`[dev-target=user-feed-type]`);
      ...
      if (data.type === 'insight') {
        ...
        if (data.company_details?.company_logo) {
          ...
          if (data.company_details && data.company_details['company-website']) {
            const imageUrl = 'https://logo.clearbit.com/' + data.company_details['company-website'];
  ...
  function paginationLogic(userFeedData: MergedResult, target: 'all' | 'following' | 'favourite') {
    let endPoint:
      | '/all-tab-insight-company-people_0'
      | '/favourite-tab-insight-company-people_0'
      | '/following-tab-insight-company-people_0';
    ...
    nextPage &&
      nextBtn.addEventListener('click', () => {
        ...
        getUserFeedData(endPoint, { type: 'all', page: curPage + 1 }, tagTarget);
      });
    prevPage &&
  ...
  function filterWrapInit(wrap: HTMLDivElement) {
    ...
    filterItems.forEach((item) => {
      const tabFilterType = item.getAttribute('dev-tab-filter') as
        ...
        | 'company';
      item.addEventListener('click', () => {
        document.querySelectorAll(`[dev-target="insight-pagination-wrapper"]`).forEach((item) => {
          item.scrollTo({
            top: 0,
            behavior: 'instant',
          });
  ...
  function filterFeed(
    ...
    const observer = new MutationObserver((mutationsList: MutationRecord[]) => {
      for (const mutation of mutationsList) {
        const tabType = (mutation.target as HTMLDivElement).getAttribute('dev-target') as string;
        const activeFilter = qs(`[dev-filter-wrap=${tabType}] [dev-tab-filter].active`);
