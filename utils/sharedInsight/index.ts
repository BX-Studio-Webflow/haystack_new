...
export async function sharedInsightPageCode({ dataSource }: { dataSource: 'live' | 'dev' }) {
  ...
  async function sharedInsightPageInit(token: string | null) {
    ...
    const introBtn = introWrap.querySelector<HTMLDivElement>('[dev-target=insight-btn]');
    ...
    try {
      ...
      if (insight) {
        ...
        introTextWrap.querySelector('[dev-target=insight-text]')!.textContent = insight.name;
        ...
        companyLink!.textContent = insight?.company_details ? insight.company_details.name : '';
  ...
  function tooltipInit() {
    ...
    document.addEventListener('mouseover', function (event) {
      let target =
        event.target instanceof HTMLElement ? event.target.closest('[dev-tooltip]') : null;
      if (!target) return;

      customTooltip.textContent =
        target.getAttribute('dev-tooltip') || 'Haystack license required to access this link';
    ...
    document.addEventListener('mouseout', function (event) {
      if (event.target instanceof HTMLElement && event.target.matches('[dev-tooltip]')) {
